...
4. The UI application runs well on node version `v24.13.1`. Run the UI application by first running `yarn` in the folder `UI` to restore modules, then run `yarn dev` in the same folder. You should be able to see the output in a browser for `http://localhost:5173/`
